...
public class Task extends Todo {
    ...
    public void setStatus(Status status) {
        ...
        } else if (status.getDescription().equals("DONE") && !this.getDescription().equals("DONE"))  {
            this.status = status;
            setProgress(100);
        }
...
//  throws InvalidProgressException if !(0 <= progress <= 100)
    public void setProgress(int progress) {
        if (progress < 0 || progress > 100) {
            throw new InvalidProgressException("The progress should be bigger or equal than 0 and smaller or equal 0");
        }
        if (progress == 100 && super.progress != 100) {
...
//  throws NegativeInputException if hours < 0
    public void setEstimatedTimeToComplete(int hours) {
        if (hours < 0) {
            throw new NegativeInputException("The hours should be greater than 0");
        }
        this.etcHours = hours;
    }
}
